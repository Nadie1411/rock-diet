...
| -------------------- | ------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-09-01T01:00:00Z | `../rock-diet-backend/src/modules/order/order.service.js`, `../rock-diet-backend/src/DB/model/order.model.js` | Fixed Stripe billing currency + coupon charge: switched Checkout line items from `currency: "aed"` with `unit_amount = KD×100` (charged ~1/12 of the shown KD price) to native `currency: "kwd"` with `unit_amount = KD×1000` (= KWD minor unit, exact 3-decimal match with DB prices); coupon discounts now actually reduce the Stripe charge via a per-order one-time `amount_off` Stripe coupon (`Math.round(discountAmount*1000)`, `duration: "once"`, `max_redemptions: 1`) passed in `discounts` instead of only lowering the DB `order.totalPrice`; webhook now stores reconciliation fields `paymentAmountKwd` (Stripe minor-unit `amount_total`) and `paymentCurrency: "kwd"` on paid orders (added optional fields to order model). |
| 2026-09-01T00:00:00Z | `../rock-diet-backend/src/modules/auth/auth.service.js`, `../rock-diet-backend/src/common/service/redis.service.js` | Capped OTP verification attempts: `confirmEmail` now enforces a max of 3 invalid attempts per email via a new Redis counter key (`otp_verify::{email}`, 300s TTL) — on the 3rd failed attempt the stored OTP is deleted and a 429 "Too many invalid attempts. Please request a new OTP." is returned; attempts are reset whenever a new OTP is issued (`sendEmailOtp`) and cleared after a successful confirm; added `MAX_OTP_VERIFY_ATTEMPTS=3` and `OTP_VERIFY_WINDOW_SECONDS=300` constants. |
| 2026-08-26T01:30:00Z | `../rock-diet-backend/src/modules/order/order.service.js`, `src/index.css`, `src/pages/Admin.jsx` | Fixed admin order notification not firing: moved `sendAdminOrderNotification` into `createOrder` (fire-and-forget with `.catch()`) so admins are notified immediately when an order is placed, not only after Stripe webhook confirmation which often doesn't fire on localhost; resized print sticker to receipt format: added `@page { size: 75mm auto; margin: 0 }`, shrunk sticker from 80mm to 73mm, scaled all fonts/spacing proportionally, removed prep notes lines from print, padding zeroed on wrapper. |
...
| 2026-08-26T00:05:00Z | `../rock-diet-backend/src/DB/model/order.model.js`, `../rock-diet-backend/src/modules/order/order.validation.js`, `../rock-diet-backend/src/modules/order/order.service.js`, `src/components/CartDrawer.jsx`, `src/pages/Orders.jsx`, `src/pages/Admin.jsx` | Added delivery time feature to checkout: new required `deliveryTime` field (String, trimmed) on the order model with Joi validation (min 3, max 50, required with custom error messages); backend `createOrder` service now accepts and persists `deliveryTime` from the request body; frontend CartDrawer checkout step gained a delivery time `<select>` placed between phone and delivery notes with 3 fixed period options (Morning 7–10 AM, Afternoon 12–3 PM, Evening 6–9 PM) plus a "Specific Time" option that reveals a native `<input type="time">` picker for custom time entry, with client-side required validation and reset on drawer close; `deliveryTime` is sent in the `POST /order` request body; displayed in user Orders active card (Clock icon + label) and Admin orders Delivery Info section. |
| 2026-08-26T00:10:00Z | `../rock-diet-backend/src/modules/user/user.validation.js`, `../rock-diet-backend/src/modules/user/user.service.js`, `../rock-diet-backend/src/modules/user/user.controller.js`, `src/services/userService.js`, `src/pages/Admin.jsx` | Added admin user creation: new `adminCreateUserSchema` Joi validation (firstName, lastName, email, phoneNumber, password with same regex as signup, age, weight, height, gender required; activityLevel default moderate; goal, package, duration optional); new `adminCreateUser` service checks email uniqueness, auto-confirms account (sets `confirmAccount` timestamp so user can log in immediately without OTP), calculates subscription dates when package+duration provided, hashes password via pre-save hook; new `POST /user/admin/create-user` admin-only route; frontend `userService.adminCreateUser` wired to the endpoint; Admin Subscribers tab gained a "Create User" button (UserPlus icon) in the header that opens a full modal form with all fields (name, email, phone, password, age/weight/height, gender, activity level, goal, optional subscription package+duration), client-side validation mirroring backend Joi, loading/error states, success feedback, auto-refreshes subscriber list on creation. Also fixed `getSubscribers` filter from `package: { $exists: true, $nin: [null, ""] }` to `{}` so admin-created users without a package appear in the list, and updated the subscriber status badge to show "No Plan" (neutral) instead of "Expired" (red) for users with no subscription. |
| 2026-08-26T00:15:00Z | `src/pages/Admin.jsx`, `src/index.css` | Added auto-print order sticker + CSV export: 15-second polling interval silently re-fetches orders and detects new ones by comparing ID sets — when a new order arrives, sets `printingOrder` state and triggers `window.print()` after 300ms delay; print-only `.print-sticker` div renders a compact 80mm receipt-style ticket (monospace font, dashed dividers) with Rock Diet header, order ID, date/time, status, customer name/phone/address/delivery time, item list, coupon/note, total price, and a lined "Prep Notes" area for kitchen staff — `@media print` CSS hides the entire Admin UI and shows only the sticker; manual Printer button added next to the status dropdown on every order card for re-printing any past order; new `exportOrdersCSV` function generates a CSV blob with 14 columns (Order ID, Date, Customer, Email, Phone, Address, Delivery Time, Items, Subtotal, Discount, Total, Status, Coupon, Note) with proper escaping and auto-downloads as `rock-diet-orders-YYYY-MM-DD.csv`; Download button added in Orders tab header next to the filter. |
